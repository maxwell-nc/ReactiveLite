 ...
 * 需要注意，由于产生数据操作是阻塞操作，需要设置调度器{@link #schedulers}防止主线程阻塞<br/>
 * 默认情况下没有设置调度器，将会在所执行的线程生产数据<br/>
 * 此生产者可以通过以下方法创建：<br/>
 * {@link Publisher#timer(long)}<br/>
 * {@link Publisher#timer(long, Schedulers)}<br/>
 */
public class TimerPublisher extends Publisher<Long> {
    ...
    public TimerPublisher(long interval, Schedulers schedulers) {
    ...
    protected void subscribeActual(Subscriber<Long> subscriber) {
        subscriber.onSubscribe(new TimerSubscription(subscriber, interval, schedulers));
    }
    ...
    private static final class TimerSubscription extends FlowSubscription implements FlowStream<Long> {
        ...
        private TimerSubscription(Subscriber<Long> subscriber, long interval, Schedulers schedulers) {
